...
function Navbar() {
  ...
  useEffect(() => {
    ...
    document.title =
      Titles[location.pathname] || Titles[`/${basePath}`] || "movie-world";
  }, [location]);
      ...
      <div className="navbar  container    mx-auto">
        <div className="navbar-start">
          ...
          <Link
            to={"/"}
            id="wname"
            className="btn text-transparent bg-clip-text bg-gradient-to-l text-xl from-red-500 to-orange-500 btn-ghost px-1 font-bold   md:text-2xl"
          >
            Movie world
          </Link>
        </div>
        ...
        <div className="navbar-end space-x-1">
          <button onClick={toggleTheme} className=" mr-1 md:mr-3">
            {theme === "light" ? (
              <MdOutlineLightMode className="text-xl text-black" />
            ) : (
              <LuMoonStar className="text-xl text-white" />
            )}
          </button>
          ...
          {user ? (
            <button
              ...
              className=" py-2 px-3 rounded-lg bg-rose-500 font-semibold text-white text-xs md:text-base"
            >
              LogOut
            </button>
          ) : !isLoading ? (
            <>
              <div className=" font-semibold space-x-1">
                <Link
                  to="/Login"
                  className="py-3 mr-1 px-4 rounded-lg bg-blue-500 text-white text-sm md:text-base"
                >
                  login
                </Link>
                <Link
                  to="/Register"
                  className="py-3 px-4 rounded-lg bg-orange-500 text-white text-sm md:text-base"
                >
                  register
                </Link>
              </div>
